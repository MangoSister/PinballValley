...
public class GatheringFlagController : MonoBehaviour
{
    ...
    private void Start()
    {
        sensitivity = Mathf.Clamp(sensitivity, 5f, 10f);
    }
    private void Update()
    {
		...
		newPos += (transform.right * horiInput  * sensitivity * Time.deltaTime);
		newPos += (transform.forward * vertInput * sensitivity * Time.deltaTime);
